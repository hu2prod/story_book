...
class @Request_cache
  ...
  # ###################################################################################################
  _map              : null
  _entity_count     : 0 # == _map key count
  _sum_access_count : 0
  ...
  # ###################################################################################################
  get : (key)->
    return null if !cache_entity = @_map.get key
    cache_entity.access_ts = Date.now()
    cache_entity.access_count++
    cache_entity.value
  
  set : (key, value)->
    if !cache_entity = @_map.get(key)
      @_map.set key, {
        access_ts   : Date.now()
        access_count: 1
        value
  ...
  # NOTE у _map это delete, но delete у нас это деструктор
  remove : (key)->
    if cache_entity = @_map.get(key)
      @_entity_count--
  ...
  # ###################################################################################################
  get_error : (key)->
    return null if !cache_entity = @_map_error.get key
    cache_entity.value
  
  set_error : (key, err)->
    @_map.delete key
    @_map_error.set key, cache_entity = {
      key
